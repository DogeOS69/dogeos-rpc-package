...
exec geth \
    ...
    --config "$CONFIG_FILE" \
    --http --http.port "$L2GETH_RPC_HTTP_PORT" --http.addr "0.0.0.0" --http.vhosts="*" --http.corsdomain '*' --http.api "eth,net,web3,debug,trace,scroll" \
    --pprof --pprof.addr "0.0.0.0" --pprof.port 6060 \
    --ws --ws.port "$L2GETH_RPC_WS_PORT" --ws.addr "0.0.0.0" --ws.api "eth,net,web3,debug,trace,scroll" \
